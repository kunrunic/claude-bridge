...
def test_parse_lock_missing(tmp_path):
    with patch.object(bot, "_lock_path", return_value=tmp_path / "nonexistent"):
...
def test_acquire_lock_duplicate_fails(tmp_path):
    ...
    with patch.object(bot, "_lock_path", return_value=lp), \
         patch.object(bot, "tmux_run", return_value=_alive_tmux()):
...
def test_acquire_lock_stale_cleanup(tmp_path):
    ...
    with patch.object(bot, "_lock_path", return_value=lp), \
         patch.object(bot, "tmux_run", return_value=_dead_tmux()):
...
def test_my_locks_empty(tmp_path):
    with patch.object(bot, "_LOCK_DIR", tmp_path):
...
def test_bridge_start_new_session_no_lock(tmp_path):
    ...
    b = bot.Bridge()
    with patch.object(bot, "tmux_run", return_value=_alive_tmux()), \
         patch.object(b, "_spawn", return_value=True):
...
def test_bridge_start_releases_previous_lock(tmp_path):
    ...
    with patch.object(bot, "_lock_path", side_effect=_lp), \
         patch.object(bot, "tmux_run", return_value=_alive_tmux()), \
         patch.object(b, "_spawn", return_value=True):
