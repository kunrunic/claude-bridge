...
# 10. user-scope MCP 등록 — cwd 무관하게 bridge-channel 을 찾게 하기 위함
...
if ( cd /tmp && claude mcp get bridge-channel ) >/dev/null 2>&1; then
  echo "✓ bridge-channel 이미 user scope 에 등록됨 (skip)"
else
  echo
  echo "registering bridge-channel as user-scope MCP server..."
...
# Source of truth for the .md body lives at bin/handoff-to-bridge.md.tpl.
